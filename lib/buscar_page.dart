...
class _BuscarPageState extends State<BuscarPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: const [
          _DebugUploadTokenButton(), // 👈 Botón nuevo de depuración
        ],
      ),
      body: Container(
                  ...
                  : StreamBuilder<List<QueryDocumentSnapshot<Map<String, dynamic>>>>(
                      ...
                      builder: (context, snapshot) {
                        ...
                        for (final d in docs) {
                          final data = d.data();
                          final nombre =
                              (data['nombre_canal'] ?? '').toString().trim();
                          final nombreLower =
                              (data['nombre_canal_lower'] ??
                                      nombre.toLowerCase())
                                  .toString()
                                  .trim();
                          ...
                          if (matches) {
                            scored.add((
                              _Canal(
                                ...
                                foto: (data['foto_canal'] ?? data['foto'])
                                        ?.toString() ??
                                    '',
                              ),
                              score
                          ...
                          itemBuilder: (context, index) {
                            ...
                            return ListTile(
                              ...
                              leading: CircleAvatar(
                                radius: 24,
                                backgroundImage: c.foto.isNotEmpty
                                    ? NetworkImage(c.foto)
                                    : null,
                              ...
                              onTap: () {
                                Navigator.push(
                                  ...
                                  MaterialPageRoute(
                                    builder: (_) =>
                                        TipsterChannelPage(tipsterId: c.id),
                                  ),
                                );
                              },
                            );
  ...
  Stream<List<QueryDocumentSnapshot<Map<String, dynamic>>>> _searchStream(
      String input) {
    ...
    final pubStream = db
        ...
        .map((s) => s.docs)
        .onErrorReturnWith(
            (_, __) => <QueryDocumentSnapshot<Map<String, dynamic>>>[]);
...
/// 👇 Nuevo botón de depuración
class _DebugUploadTokenButton extends StatelessWidget {
  const _DebugUploadTokenButton();

  @override
  Widget build(BuildContext context) {
    return IconButton(
      icon: const Icon(Icons.cloud_upload),
      tooltip: "Subir token a Firestore",
      onPressed: () async {
        String? fcm;
        String? apns;
        try {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              fcm == null
                  ? "❌ No hay FCM token (subido null)"
                  : "✅ Token subido: ${fcm.substring(0, 8)}…",
            ),
          ),
        );
      },
