...
class _TipsterChannelPageState extends State<TipsterChannelPage> {
  ...
  /// ✅ sumar 1 vista: añade TU uid a `viewers` 1 sola vez
  ...
  Widget _statusLabel(String status) {
    Color bg;
    String text;
    if (status == 'won') { bg = Colors.green; text = "GANADA ✅"; }
    else if (status == 'lost') { bg = Colors.red; text = "PERDIDA ❌"; }
    else { bg = Colors.orange; text = "EN JUEGO 🟡"; }
    return Container(
  ...
  Stream<List<Map<String, dynamic>>> _combinedStream() {
    ...
    return canalRef.snapshots().switchMap((canalDoc) {
      ...
      // Pedimos todos los posts; reglas decidirán qué se puede leer
      final Stream<QuerySnapshot<Map<String, dynamic>>> postsSnapStream =
          postsCol.orderBy('postedAt').snapshots();

      // ❌ sin orderBy('fecha') para evitar índice compuesto
      final Stream<QuerySnapshot<Map<String, dynamic>>> apuestasSnapStream =
          db.collection("apuesta_resuelta")
        ...
        children: [
          ...
          StreamBuilder<DocumentSnapshot>(
            ...
            builder: (ctx, snap) {
              ...
              final numeroSeguidores = (canal['numero_seguidores'] is num) ? canal['numero_seguidores'] : seguidores.length;
